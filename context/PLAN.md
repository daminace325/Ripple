...
> Updated: 2026-07-07 — Legend: **Done** / **In progress** / **Not started** / **Deferred**
...
- `backend/app/config.py` — pydantic-settings; required `DATABASE_URL` + `JWT_SECRET_KEY` (+ `redis_url`, `timeline_max_size`/`timeline_ttl_seconds`, `feed_stream_maxlen`, `post_cache_ttl_seconds`, `engagement_count_ttl_seconds`, `fanout_chunk_size`/`worker_batch_size`/`worker_block_ms`, `db_pool_size`/`db_max_overflow`/`db_pool_timeout`/`db_pool_recycle`/`redis_max_connections`, `celebrity_threshold`/`celebrity_cache_size`, `follower_count_ttl_seconds`, `feed_backend` (naive-Postgres vs Redis feed, for benchmarking), `jwt_algorithm`, `access_token_expire_minutes`, `cors_origins`)
...
- `backend/app/services/celebrities.py` — O(1) celebrity classification: cached follower count `user:{id}:followers` (backfilled from Postgres on miss, maintained on follow/unfollow), `is_celebrity` vs `celebrity_threshold`; batched `get_follower_counts` (one `MGET`) classifies all followees per feed read; recent-posts cache `celebrity:{id}:posts` ZSET (`add_recent_post`/`get_recent_post_ids`, backfilled on miss) [3.1/3.3/4]
- `backend/app/services/posts.py` — create post, list a user's posts (newest-first)
- `backend/app/services/likes.py` — like/unlike (idempotent `ON CONFLICT`); Postgres counts + **Redis engagement counters** (`post:{id}:likes`, `INCR`/`DECR`-if-present, backfilled from Postgres on a miss, self-healing TTL) [1.12/4]
- `backend/app/services/comments.py` — create/list comments; Postgres counts + **Redis engagement counters** (`post:{id}:comments`, backfilled on a miss, self-healing TTL) [1.14/4]
- `backend/app/services/feed.py` — hybrid home feed: merges the Redis timeline ZSET (own + **normal** followees, rebuild-on-miss/TTL) with followed celebrities' cached recent posts at read time (`get_feed_page`), keyset-paginated by post id; post-body hydration via a Redis `post:{id}` cache (`MGET` cache-aside, one round-trip on a warm cache); also a **naive Postgres baseline** (`get_feed_page_naive`/`hydrate_posts_db`) selectable via `feed_backend`/`?backend=` for before/after benchmarking [4.7]
...
- `backend/app/routers/auth.py` — `POST /auth/register`, `POST /auth/login` (email); `backend/app/routers/users.py` — `GET /users/me`, `PATCH /users/me`, `GET /users/search`, `GET /users/{id}`, `GET /users/by-username/{username}` (profile + counts); `backend/app/routers/follows.py` — `POST`/`DELETE /follow`; `backend/app/routers/posts.py` — `POST /posts` (enqueues fan-out), `GET /posts/{id}` (detail + author + likes), `GET /users/{id}/posts` (enriched, auth), `POST`/`DELETE /posts/{id}/like`, `GET`/`POST /posts/{id}/comments`; `backend/app/routers/feed.py` — `GET /feed` (Redis timeline; items include author + like/comment counts; `?backend=postgres|redis` overrides `feed_backend` for benchmarking)
- `backend/scripts/seed_demo.py` / `unseed_demo.py` — small demo dataset for hand-browsing: 20 named users under `@test.com` (primary login `damin@test.com`/`damin123`), a follow graph + posts (`python -m scripts.seed_demo`); `backend/scripts/seed_loadtest.py` / `unseed_loadtest.py` — large synthetic dataset for benchmarking: `seeduser{i}@example.com` (`python -m scripts.seed_loadtest [--users --posts --follows]`, idempotent, shared bcrypt hash); `backend/scripts/loadtest.py` — async+httpx feed load-test harness (logs in the seeded users, concurrency sweep → rps + p50/p95/p99) [4.6]; `backend/scripts/bench_celebrity.py` — celebrity fan-out **write-amplification** benchmark: seeds an author + N followers, then counts follower-timeline writes for a normal fan-out vs the celebrity path (→ "writes avoided per post"; verified O(followers)→O(1), e.g. 10k→0 at 10k followers)
- `backend/alembic/` + `alembic.ini` — Alembic (async); migrations: `dcfce07fa8f2` (schema), `30f2d801d8cb` (password_hash), `53dcc349a3d9` (email + nullable username), `0be43df3a9c7` (likes), `80080e70e043` (comments), `79eaa3794e1d` (follows.followee_id index)
...
- `backend/tests/` + `pytest.ini` — pytest + httpx integration suite (conftest fixtures + 10 modules, 67 tests); runs against an auto-created `<db>_test` database with per-test schema rebuild
...
user:{id}:followers  -> cached follower count (TTL, self-healing)
celebrity:{id}:posts -> ZSET of a celebrity's recent post ids (read-time merge)
post:{id}            -> JSON post body (content + author), TTL (feed hydration)
post:{id}:likes      -> cached like count (INCR/DECR-if-present, self-healing TTL)
post:{id}:comments   -> cached comment count (INCR-if-present, self-healing TTL)
```
...
- **1.8 — Seed script** — generate N users (with passwords), a random follow graph, and posts for local testing/benchmarking. _Done when:_ one command populates a demo dataset. _(Delivered as two pairs: `scripts/seed_demo.py`+`unseed_demo.py` — 20 named users under `@test.com` (primary `damin@test.com`/`damin123`) for hand-browsing; `scripts/seed_loadtest.py`+`unseed_loadtest.py` — `python -m scripts.seed_loadtest [--users --posts --follows]`, large synthetic `seeduser*@example.com` dataset for benchmarking, idempotent, shared bcrypt hash. Each cleaned up by its `unseed_*` counterpart.)_ **[DONE]**
...
- **4.2 — Batch hydration + post cache** ✅ — feed post bodies are hydrated from a Redis `post:{id}` cache via a single `MGET` (cache-aside: misses batch-load from Postgres and write back with `post_cache_ttl_seconds`). On a warm cache, hydration is one Redis round-trip and zero Postgres. Post bodies are immutable, so the only staleness is an author renaming (bounded by TTL). _Done when:_ feed hydration is one round-trip per page. _(Delivered: `feed.hydrate_posts` rewrite + `HydratedPost`, router builds items from it; 1 test; 61 total. Like/comment counts were subsequently moved to **Redis engagement counters** (`post:{id}:likes`/`:comments`, backfilled on a miss) so a warm feed read needs zero Postgres count queries.)_
